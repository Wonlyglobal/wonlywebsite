...
begin
 ...
 with days as(select generate_series(v_since::date,current_date,'1 day')::date event_day),filtered as(select*from public.cms_search_events where searched_at>=v_since and(v_locale is null or locale=v_locale)and(v_query is null or position(v_query in lower(query_text))>0)),searches as(select searched_at::date event_day,count(*)searches,count(*)filter(where result_count=0)zero_results from filtered group by 1),clicks as(select c.clicked_at::date event_day,count(*)clicks from public.cms_search_clicks c join filtered f on f.id=c.search_event_id group by 1)
 select coalesce(jsonb_agg(jsonb_build_object('date',d.event_day,'searches',coalesce(s.searches,0),'zero_results',coalesce(s.zero_results,0),'clicks',coalesce(c.clicks,0))order by d.event_day),'[]')into v_trend from days d left join searches s using(event_day)left join clicks c using(event_day);
