...
type SelectorGroup = "security" | "wooden";
...
export function DoorModelSelector({ group, currentPath }: { group: SelectorGroup; currentPath: string }) {
  const models = group === "security" ? SECURITY_MODELS : WOODEN_MODELS;
  return <section className="px-[6vw] py-20 md:py-28 bg-white" aria-labelledby={`${group}-door-selector-title`}>
    <div className="max-w-[1450px] mx-auto">
      <div className={eyebrow} style={{ color: GOLD_DEEP }}>Door Selection Guide</div>
      <h2 id={`${group}-door-selector-title`} className={h2cls + " mt-4 max-w-4xl"}>{group === "security" ? "Compare WONLY Security Door Models" : "Choose the Right Wooden Door Series"}</h2>
