...
export async function submitEnquiry(payload: Record<string, string>): Promise<{ success: boolean; message?: string }> {
  ...
  if (result.success && cmsSupabase) {
    const { error } = await cmsSupabase.from("cms_inquiries").insert({
      ...
      business_type: payload.business_type || "", message: payload.message || "", language: payload.language || document.documentElement.lang || "en",
